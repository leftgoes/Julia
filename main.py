...
class Julia:
    def __init__(self, f: Callable, width: int = 640, c: complex = None, mandelbrot: bool = False, real: tuple[float, float] = (-2, 2), imag: tuple[float, float] = (-2, 2), max_iterations: int = 30, extra_iterations: int = 2, max_magnitude: float = 2, threads: int = 8, info: bool = True, exponent: int = 2, oversample: int = 1, square_tiling: bool = True, check: bool = False):
        if not mandelbrot and c is None:
            raise TypeError("'Julia' missing required positional argument: 'c'")

        self.f = f
        self.c = c
        self.w = width
        self.real = real if real[1] > real[0] else real[::-1]  # make sure ranges are sorted
    ...
    def h(self, w: int = None, real: tuple = None, imag: tuple = None):
    ...
    def get_render_areas(self) -> list[tuple[tuple, tuple]]:
        self.sort()
        re_1, re_2 = self.real
        im_1, im_2 = self.imag
        im_long = max(abs(im_1), abs(im_2))
        if (np.sign(im_1) != np.sign(im_2) and (self.mandelbrot or ((not self.mandelbrot) and np.sign(re_1) != np.sign(re_2)))):  # cannot exploit symmetry
            imag = (im_long, 0)
            if self.mandelbrot or (not self.mandelbrot and re_1 == -re_2):
    ...
    def type(self) -> str:
        ...
        for line in lines:
            if line.find('return') != -1:
                ...
                return f"Mandelbrot; {text}" if self.mandelbrot else f'Julia; {text.replace(", c", "")}'
        return 'Mandelbrot' if self.mandelbrot else 'Julia'
    ...
    def continuous(self, z: complex, i: int) -> float:  # discrete iteration value to continuous
        if i == 0:
            return 0.0
        else:
    ...
    def get_squares(self, w: int, h: int, x0: int = 0, y0: int = 0, size_offset: int = None) -> list:
        if w == 0 or h == 0:
            return []
        elif w == h == 1:
            return [((x0, y0), 1)]
    ...
    def calculate_square(self, zeros: set, nonzeros: dict, x0: int, y0: int, size: int) -> tuple[dict, list]:
        ...
        for x, y in left + right + top + bottom:
            ...
            z, iterations = self.method.calculate_i(z)
            if iterations == 0:
                zeros.update((x, y))
            else:
                if self.check:
    ...
    def thread_tiling(self, n: int) -> tuple[np.ndarray, list]:  # thread with square_tiling enabled
        ...
        if self.info and n == check_n:
            print(f'\r[INFO] calculate | {functions.progressbar(progress_total, delta_progressbar)} | combining arrays..{60*" "}', end='')
        main_arr = arrays[0][0]
        if len(arrays) == 1:
            if self.real[0] == -self.real[1]:
                arr = np.concatenate((main_arr, main_arr[::-1, ::-1]))
            else:
                arr = main_arr
        else:
    ...
    def calculate(self, size_offset: int = 0) -> float:
        self.method = CalculateMandelbrot(self.f, self.max_iterations, self.max_magnitude, self.extra_iterations) if self.mandelbrot else CalculateJulia(self.f, self.c, self.max_iterations, self.max_magnitude, self.extra_iterations)
        self.checked = []
